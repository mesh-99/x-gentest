...
func processXRDFiles(files []string, outputFolder string) error {
    for _, m := range files {
        fmt.Println(m)
        ...
        output := filepath.Join(outputFolder, "/package/crds/", fmt.Sprintf("%s_%s.yaml", crd.Spec.Group, crd.Spec.Names.Plural))
        fmt.Printf("Output path: %s\n", output)
